...
public:
	...
	{
		for (unsigned int i = 0; i < meshes.size(); i++) { meshes[i].Draw(shaderProgram); }
	...
	void Render(const glm::mat4&  projection, const glm::mat4&  modelview, GLuint shaderProgram) {
		...
		//glm::mat4 t = glm::translate(modelview,glm::vec3(0.0f,0.0f,-0.0f));
...
private:
	...
	Mesh processMesh(aiMesh *mesh, const aiScene *scene)
	{
		...
		for (unsigned int i = 0; i < mesh->mNumVertices; i++)
		{
			...
			cout << "pos x y z" << vector.x <<" " << vector.y <<" "<< vector.z << endl;
			vertex.Position = vector;
			// normals
			vector.x = mesh->mNormals[i].x;
			vector.y = mesh->mNormals[i].y;
			vector.z = mesh->mNormals[i].z;
			cout << "normals x y z" << vector.x <<" "<< vector.y <<" "<< vector.z << endl;
